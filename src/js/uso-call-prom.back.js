...
Promise.all([buscarHeroe(heroeId1), buscarHeroe(heroeId2)])
    .then( ([heroe1, heroe2]) =>{
        //este codigo se ejecuta si todas las promesas se cumplen
        console.log(`Enviando a ${heroe1.nombre} y ${heroe2.nombre} a la misión`);
}).catch(err => {//si falla una ignora el resto y ejecuta el siguiente codigo
    alert(err);
}).finally( ()=>{//se ejecuta cuando todo termino
